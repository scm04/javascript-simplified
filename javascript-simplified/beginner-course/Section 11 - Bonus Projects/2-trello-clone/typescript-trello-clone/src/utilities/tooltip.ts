...
addGlobalEventListener("mouseover", "[data-tooltip]", e => {
	...
	// (DONE) Generalize this observer behavior and add it to the original tooltip library as an optional feature.
	...
	// TODO: TLDR; I am unhappy with the current observer solution for when an element is deleted because it requires
	// the user to add another data attribute to an unrelated element in order to properly clean up the tooltip. I
	// would like to try to come up with another solution that doesn't involve just observing the document or body.
	// Full Explanation: I think I could do this differently by having the tooltip attach itself to the target element.
	// This could be achieved by giving each tooltip element an id using the UUID library and adding a data attribute
	// [data-tooltip-id="<uuid>"] to the target element. <uuid> would be the id generated by UUID run through CSS.escape(),
	// then, when the target element is removed from the DOM, it can call a handler to remove the tooltip as well.
	// The biggest downside to this is that the tooltip logic is no longer self-contained, because even if the cleanup code
	// is contained within the library, it will have to be triggered by the user.
	// I really don't like the way this is handled now, but I'm not sure how to modify it without really putting
	// the logic outside of the library or doing something like making the observer observe the document or some
	// other very high-level element that I know will never be removed. In a perfect world, no data attributes would
	// be needed aside from the tooltip attribute that sets the text of the tooltip, but I'm having a hard time
	// seeing how that would be done.
	/**
	 * The "mouseleave" event is not triggered by an element being removed from the DOM, so if a tooltip
	 * is applied to an element that will be removed, add the [data-tooltip-observe] attribute to one of
	 * its ancestors so that the tooltip will be properly removed when the element is deleted. NOTE: the
	 * data attribute must be added to an ancestor that will still exist in the DOM after the element with
	 * the tooltip has been removed.
	 */
	const elementToObserve: HTMLElement | null = target.closest("[data-tooltip-observe]")
	let observer = null
